...
def Traduce_posiciones(VA, VH):
    ...
    for event in eventos:
        evt = pygame.event.Event(
            pygame.MOUSEBUTTONDOWN, pos=event_pos, button=event.button)
    ...
    for event in eventos:
        evt = pygame.event.Event(
            pygame.MOUSEMOTION, pos=event_pos, rel=event.rel, buttons=event.buttons)
        pygame.event.post(evt)
...
class Main():
    ...
    def load(self):
        ...
        if not self.fondo:
            self.fondo = self.get_fondo(color=b)
        ...
        if not self.controles:
            self.controles = pygame.sprite.OrderedUpdates()
        if not self.cerrar:
            self.cerrar = JAMButton("", JAMG.get_icon_exit())
            self.cerrar.set_imagen(
                origen=JAMG.get_icon_exit(), tamanio=(20, 20))
        ...
        if not self.jamcalendar:
            ...
            self.jamcalendar.set_gama_colors(
                colorselect=a, colorbor=b, colorcara=c)
    ...
    def load_conf(self):
        ...
        self.controlalarma.etiqueta_tiempo.set_text(texto="%s:%s" % (
    ...
    def active_alarma(self, button=None):
        ...
        if self.controlalarma.active:
            self.jamclock.set_alarma(
                (100, 100), self.sonidoalarma, self.duracionalarma)
            self.controlalarma.set_active(False)
        elif not self.controlalarma.active:
            self.jamclock.set_alarma(
                (horas, minutos), self.sonidoalarma, self.duracionalarma)
...
class ControlAlarma(pygame.sprite.OrderedUpdates):
    ...
    def set_active(self, valor):
        if valor:
            ...
            self.boton_active.set_imagen(
                origen=JAMG.get_icon_ok(), tamanio=(20, 20))
        elif not valor:
            self.active = False
            self.boton_active.set_imagen(
    ...
    def next(self, button=None):
        ...
        if self.horas >= 24:
            self.horas = 0
        self.etiqueta_tiempo.set_text(
            texto="%s:%s" % (self.horas, self.minutos))
    ...
    def load(self):
        ...
        self.etiqueta_tiempo.set_tamanios(
            tamanio=(0, 0), grosorbor=1, espesor=1)
        ...
        self.botonminutos_down.set_tamanios(
            tamanio=(0, 0), grosorbor=1, espesor=1)
        ...
        self.botonminutos_up.set_tamanios(
            tamanio=(0, 0), grosorbor=1, espesor=1)
        ...
        self.botonhoras_down.set_tamanios(
            tamanio=(0, 0), grosorbor=1, espesor=1)
        ...
        self.etiqueta_tiempo.set_tamanios(
            tamanio=(w, 0), grosorbor=1, espesor=1)
    ...
    def set_posicion(self, punto=(0, 0)):
        ...
        # ---
        xx = self.base.rect.x + self.base.rect.w / \
            2 - self.etiqueta.get_tamanio()[0]/2
        self.etiqueta.set_posicion(punto=(xx, y))

        y += self.etiqueta.get_tamanio()[1] + sep
        xx = self.base.rect.x + self.base.rect.w/2 - \
            self.etiqueta_tiempo.get_tamanio()[0]/2
        self.etiqueta_tiempo.set_posicion(punto=(xx, y))
        # ---

        x, y = self.etiqueta_tiempo.get_posicion()
        xx, hh = self.botonminutos_up.get_tamanio()
        x += self.etiqueta_tiempo.get_tamanio()[0] - \
            self.botonminutos_up.get_tamanio()[0]
        y -= self.botonminutos_up.get_tamanio()[1] - \
            self.etiqueta_tiempo.get_tamanio()[1]/2
