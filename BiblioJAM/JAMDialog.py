...
class JAMDialog(pygame.sprite.OrderedUpdates):
    def __init__(self, mensaje="JAMDialog", funcion_ok=None, funcion_cancel=None):
        ...
        # self.set_center()
        self.JAMObjects = {"base": self.base, "etiqueta": self.label,
                           "botonaceptar": self.boton_aceptar, "botoncancelar": self.boton_cancelar}
    ...
    def set_text(self, tipo=None, tamanio=None, color=None, texto=None):
        ...
        self.label.set_text(tipo=tipo, tamanio=tamanio,
    ...
    def set_text_buttons(self, tipo=None, tamanio=None, color=None, textoaceptar=None, textocancelar=None):
        ...
        self.boton_aceptar.set_text(
            tipo=tipo, tamanio=tamanio, color=color, texto=textoaceptar)
        self.boton_cancelar.set_text(
            tipo=tipo, tamanio=tamanio, color=color, texto=textocancelar)
    ...
    def set_colors_buttons(self, colorbas=None, colorbor=None, colorcara=None):
        ...
        self.boton_aceptar.set_colores(
            colorbas=colorbas, colorbor=colorbor, colorcara=colorcara)
        self.boton_cancelar.set_colores(
            colorbas=colorbas, colorbor=colorbor, colorcara=colorcara)
    ...
    def connect(self, funcion_ok=None, funcion_cancel=None):
        ...
        if funcion_ok != self.funcion_ok:
            ...
            self.boton_aceptar.connect(
                callback=self.funcion_ok, sonido_select=VG.get_sound_select())
        if funcion_cancel != self.funcion_cancel:
            self.funcion_cancel = funcion_cancel
            self.boton_cancelar.connect(
                callback=self.funcion_cancel, sonido_select=VG.get_sound_select())
    ...
    def get_label(self):
        ...
        label.set_contenedor(
            colorbas=self.colores["base"], grosor=None, colorbor=None)
        return label
    ...
    def get_base(self):
        ...
        tamanio_frame_interno = (
            self.grosorbor_int*2+w+self.separador*2, self.grosorbor_int*2+self.separador*2+h)
        frame1 = VG.get_Rectangulo(self.colores["base"], tamanio_frame_interno)
        frame1 = VG.get_my_surface_whit_border(
            frame1, self.colores["bordes"], self.grosorbor_int)
        ...
        frame2 = VG.get_my_surface_whit_border(
            frame2, self.colores["bordes"], self.grosorbor_med)
        frame2.blit(frame1, (self.separador, self.separador))

        ww, hh = frame2.get_size()
        # Recuadro exterior - imagen final de la base
        tamanio_frame_externo = (
            ww+self.grosorbor_ext*3, hh+self.grosorbor_ext*3)
        base = VG.get_Rectangulo(self.colores["base"], tamanio_frame_externo)
        base = VG.get_my_surface_whit_border(
            base, self.colores["bordes"], self.grosorbor_ext)
...
class Ejemplo(object):
    ...
    def setup(self):
        ...
        self.widgets = JAMDialog(
            mensaje="Prueba JAMDialog", funcion_ok=None, funcion_cancel=None)
    ...
    def ejemplo_cambia_colors_Dialog(self):
        ...
        colores = [(10, 20, 100, 255), (128, 128, 128, 255),
                   (255, 255, 255, 255)]
        color = random.choice(colores)
    ...
    def ejemplo_cambia_colors_Buttons(self):
        ...
        if modificar == "colorbor":
            self.widgets.set_colors_buttons(
    ...
    def ejemplo_cambia_imagen_en_Buttons(self):
        ...
        if modificar == "tamanio":
            tamanios = [(10, 20), (30, 200), (250, 100)]
            tamanio = random.choice(tamanios)
            self.widgets.set_imagenes_buttons(
                imagenceptar=None, imagencancelar=None, tamanio=tamanio)
        if modificar == "origen":
            origenes = [VG.get_jamimagenes()[0], VG.get_jamimagenes()[1], -1]
            origen = random.choice(origenes)
            self.widgets.set_imagenes_buttons(
                imagenceptar=origen, imagencancelar=origen, tamanio=None)
        return 0
