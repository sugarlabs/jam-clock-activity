...
class JAMNubes(pygame.sprite.OrderedUpdates):
    ...
    def __init__(self, intensidad=5, rectangulo=(0, 0, 1200, 300)):
        ...
        self.suelo = self.rectangulo[1] + \
            self.rectangulo[3] + self.rectangulo[3]/3
    ...
    def genera_nube(self):
        ...
        y = random.choice(list(
            range(self.rectangulo[1]-int(h/3 * 2), self.rectangulo[1]+self.rectangulo[3]-h)))
...
class Nube(pygame.sprite.Sprite):
    ...
    def update(self):
        ...
        if self.cuenta == self.latencia:
            self.cuenta = 0
            self.set_posicion(
                punto=(self.posicion[0] + self.velocidad, self.posicion[1]))
            if not self.rect.colliderect(self.efecto.rectangulo):
                ...
                if self.efecto.lluvia:
                    self.efecto.add(Gota(self.efecto, self.rect))
