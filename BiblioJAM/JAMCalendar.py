...
class JAMCalendar(pygame.sprite.OrderedUpdates):
    ...
    def __init__(self):
        ...
        self.datos_fechas_text = {"tipo": pygame.font.get_default_font(
        ), "tamanio": 16, "color": VG.get_negro(), "font_from_file": False}
        self.datos_fechas_color = {"colorselect": COLORBAS,
                                   "colorcara": COLORCARA, "colorborde": COLORBOR}

        self.datos_dias_text = {"tipo": pygame.font.get_default_font(
        ), "tamanio": 16, "color": VG.get_negro(), "font_from_file": False}
        self.datos_dias_color = {"colorselect": COLORBAS,
                                 "colorcara": COLORCARA, "colorborde": COLORBOR}

        self.datos_cabecera_text = {"tipo": pygame.font.get_default_font(
        ), "tamanio": 16, "color": VG.get_negro(), "font_from_file": False}
        self.datos_cabecera_color = {
            "colorselect": COLORBAS, "colorcara": COLORCARA, "colorborde": COLORBOR}

        self.datos_base = {"colorborde": COLORBOR, "colorbase": COLORBAS,
                           "retiro1": 2, "grosorborde1": 4, "retiro2": 1, "grosorborde2": 2}

        self.posicion = (0, 0)

        self.anio, self.mes, self.dia = str(
            datetime.date.today()).split("-")  # el dia de hoy
    ...
    def reset_tamanios(self):
        ...
        for boton in botones:
            boton.set_tamanios(tamanio=(0, 0), grosorbor=1,
                               detalle=1, espesor=1)
            ...
            if h > lado:
                lado = h
        for boton in botones:
            boton.set_tamanios(tamanio=(lado, lado),
                               grosorbor=1, detalle=1, espesor=1)
        ...
        for boton in botones_cabecera:
            ...
            if h > lado:
                lado = h
        botones_cabecera[0].set_tamanios(
            tamanio=(h, h), grosorbor=1, detalle=1, espesor=1)
        a, b = botones_cabecera[0].get_tamanio()
        botones_cabecera[1].set_tamanios(
            tamanio=(ancho_cabecera-a*2, h), grosorbor=1, detalle=1, espesor=1)
        botones_cabecera[2].set_tamanios(
            tamanio=(h, h), grosorbor=1, detalle=1, espesor=1)
    ...
    def get_base(self):
        ...
        base.image = VG.get_my_surface_whit_border(
            base.image, colorborde, grosorborde2)
    ...
    def set_posicion(self, punto=(0, 0)):
        if type(punto) == tuple and len(punto) == 2:
            if type(punto[0]) == int and type(punto[1]) == int:
                ...
                # cabecera
                posicion = (self.posicion[0]+margen1+margen2,
                            self.posicion[1]+margen1+margen2)
    ...
    def set_calendar(self, mes, anio):
        ...
        if not type(mes) == int or not type(anio) == int or mes not in list(range(1, 13)) or not anio > 0:
            return
...
class Botones_Fechas(pygame.sprite.OrderedUpdates):
    ...
    def set_text(self):
        ...
        for linea in self.matriz_botones:
            ...
            for boton in linea:
                ...
                boton.set_text(tipo=tipo, tamanio=tamanio,
                               color=color, texto=texto)
                if fuente:
    ...
    def set_colors(self):
        ...
        for linea in self.matriz_botones:
            for boton in linea:
                boton.set_colores(colorbas=colorbas,
                                  colorbor=colorbor, colorcara=colorcara)
...
class Cabecera(pygame.sprite.OrderedUpdates):
    ...
    def get_matriz_botones(self):
        ...
        self.boton_previous.connect(
            callback=self.get_previous_mes, sonido_select=None)
    ...
    def Run(self):
        ...
        while self.nivel == "menu_0":
            ...
            if contador == 100:
                ...
                self.widgets.set_text_cabecera(
                    tamanio=50, color=(0, 0, 0, 255))
                a, b, c = VG.get_estilo_naranja()
                #self.widgets.set_colors_base(colorbase= b, colorborde= a)
                self.widgets.set_gama_colors(
                    colorselect=b, colorbor=a, colorcara=c)
