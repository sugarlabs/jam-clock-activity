...
class JAMatrix():
    ...
    def setup(self):
        ...
        if not self.fondo:
            self.fondo = self.get_fondo(
                color=VG.get_negro(), tamanio=self.resolucion)  # superficie
        ...
        if not self.posicion_label:
            self.posicion_label = (
                self.resolucion[0]/2 - self.etiqueta.rect.w/2, self.resolucion[1]/2 - self.etiqueta.rect.h/2)
    ...
    def genera_terrones(self):
        ''' Nace un Terron. '''
        if not self.imagen_matrix:
            return
        x = random.randrange(
            0, self.resolucion[0], self.imagen_matrix.get_size()[0])
        terron = Terron(self)
...
class Ejemplo(object):
    def __init__(self):
        ...
        # si quieres detectar eventos durante JAMatrix se ejecuta para cortar la carga.
        self.matrix.set_callback_event(self.handle_event)
