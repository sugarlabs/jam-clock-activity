...
class JAMBoard(pygame.sprite.OrderedUpdates):
    ...
    def set_estilo(self, board, colorbase, colorborde, colorcara):
        ...
        for color in [colorbase, colorborde, colorcara]:
            if type(color) != tuple or len(color) != 4:
                return
            for valor in color:
                if type(valor) != int:
                    return
    ...
    def set_text(self, tipo=None, tamanio=None, color=None):
        ...
        if type(color) == tuple:
            cambios = True
        if cambios:
            self.board_simbolos.set_text(
                tipo=tipo, tamanio=tamanio, color=color)  # , texto=None)
            self.board_letras.set_text(
                tipo=tipo, tamanio=tamanio, color=color)  # , texto=None)
            self.board_numeros.set_text(
                tipo=tipo, tamanio=tamanio, color=color)  # , texto=None)
            self.board_matematicas.set_text(
                tipo=tipo, tamanio=tamanio, color=color)  # , texto=None)
            self.board_otros.set_text(
                tipo=tipo, tamanio=tamanio, color=color)  # , texto=None)
            self.board_especiales.set_text(
                tipo=tipo, tamanio=tamanio, color=color)  # , texto=None)
    ...
    def set_center(self, punto=None):
        ...
        if not punto or type(punto) != tuple or type(punto[0]) != int or type(punto[1]) != int:
            w, h = (pygame.display.Info().current_w,
            ...
            posicion = (punto[0]-self.frame.rect.w/2,
                        punto[1]-self.frame.rect.h/2)
            self.set_posicion(punto=posicion)
    ...
    def set_posicion(self, punto=(0, 0)):
        ...
        if type(punto) == tuple and type(punto[0]) == int and type(punto[1]) == int:
            ...
            posicion = (self.xysimbolos[0]+self.posicion[0],
                        self.xysimbolos[1]+self.posicion[1])
            self.board_simbolos.set_posicion(punto=posicion)
            posicion = (self.xyletras[0]+self.posicion[0],
                        self.xyletras[1]+self.posicion[1])
            self.board_letras.set_posicion(punto=posicion)
            posicion = (self.xynumeros[0]+self.posicion[0],
                        self.xynumeros[1]+self.posicion[1])
            self.board_numeros.set_posicion(punto=posicion)
            posicion = (
                self.xymatematicas[0]+self.posicion[0], self.xymatematicas[1]+self.posicion[1])
            self.board_matematicas.set_posicion(punto=posicion)
            posicion = (self.xyotros[0]+self.posicion[0],
                        self.xyotros[1]+self.posicion[1])
            self.board_otros.set_posicion(punto=posicion)
            posicion = (
                self.xyespeciales[0]+self.posicion[0], self.xyespeciales[1]+self.posicion[1])
    ...
    def get_frame(self):
        # recuadro interior
        sep = self.grosor_borde1 + self.separador1 + \
            self.grosor_borde2 + self.separador2
        ...
        w = (self.grosor_borde2 + self.separador2) * \
            2 + espacio1 + self.separador1 + espacio2
        h = self.board_simbolos.frame.rect.h + \
            (self.grosor_borde2 + self.separador2)*2 + \
            self.separador1 + self.board_letras.frame.rect.h

        # posicion para board letras
        self.xysimbolos = (sep, sep)
        self.xynumeros = (
            self.xysimbolos[0] + self.board_simbolos.frame.rect.w + self.separador1, self.xysimbolos[1])
        self.xymatematicas = (
            self.xynumeros[0], self.xynumeros[1] + self.board_numeros.frame.rect.h + self.separador1)

        self.xyletras = (self.xysimbolos[0], self.xysimbolos[1] +
                         self.board_simbolos.frame.rect.h + self.separador1)

        self.xyotros = (self.xyletras[0] + self.board_letras.frame.rect.w + self.separador1,
                        self.xyletras[1] + self.board_letras.frame.rect.h - self.board_otros.frame.rect.h)

        self.xyespeciales = (self.xyletras[0] + self.board_letras.frame.rect.w + self.separador1,
                             self.xyotros[1] - self.separador1 - self.board_especiales.frame.rect.h)

        tamanio = (w, h)
        superficie = VG.get_Rectangulo(self.colores["base"], tamanio)
        superficie = VG.get_my_surface_whit_border(
            superficie, self.colores["bordes"], self.grosor_borde2)

        # recuadro exterior
        tamanio = (w + (self.grosor_borde1 + self.separador1)*2,
                   h + (self.grosor_borde1 + self.separador1)*2)
        superficie1 = VG.get_Rectangulo(self.colores["base"], tamanio)
        superficie1 = VG.get_my_surface_whit_border(
            superficie1, self.colores["bordes"], self.grosor_borde1)
...
class Ejemplo(object):
    ...
    def ejemplo_cambia_estilos(self):
        import random
        estilos = [VG.get_estilo_naranja(), VG.get_estilo_gris(),
                   VG.get_estilo_celeste()]
    ...
    def ejemplo_cambia_texto_en_JAMBoard(self):
        ...
        if modificar == "tipo":
            ...
            self.widgets.set_text(tipo=random.choice(
                tipos), tamanio=None, color=None)
    ...
    def ejemplo_cambia_colors_Board(self):
        ...
        self.widgets.set_color_base(
            random.choice(colores), random.choice(colores))
