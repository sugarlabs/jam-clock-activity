...
class JAMBoardEntryText(pygame.sprite.OrderedUpdates):
    def __init__(self):
        ...
        self.JAMObjects = {"Buffer": self.text_buffer, "Frame": self.frame_entry,
                           "Board": self.board, "EntryText": self.entry}
    ...
    def set_posicion(self, punto=(0, 0)):
        ...
        if type(punto) == tuple and type(punto[0]) == int and type(punto[1]) == int:
            ...
            x, y = (self.posicion[0], self.posicion[1] +
                    self.frame_entry.rect.h - self.board.grosor_borde1/2)
            self.board.set_posicion(punto=(x, y))
            centro = (self.frame_entry.rect.centerx,
                      self.frame_entry.rect.centery)
            x, y = (centro[0]-self.entry.frame.rect.w/2,
                    centro[1]-self.entry.frame.rect.h/2)
    ...
    def set_center(self, punto=None):
        ...
        if not punto or type(punto) != tuple or type(punto[0]) != int or type(punto[1]) != int:
            ...
            posicion = (w/2-self.frame_entry.rect.w/2, h/2 -
                        self.frame_entry.rect.h/2-self.board.frame.rect.h/2)
            self.set_posicion(punto=posicion)
        elif type(punto) == tuple and type(punto[0]) == int and type(punto[1]) == int:
            posicion = (punto[0]-self.frame_entry.rect.w/2, punto[1] -
                        self.frame_entry.rect.h/2-self.board.frame.rect.h/2)
            self.set_posicion(punto=posicion)
    ...
    def set_estilo(self, colorbase, colorborde, colorcara):
        ...
        self.entry.set_entry(tipo_letra=None, tamanio_letra=None,
                             color_texto=None, color_fondo=colorborde)
    ...
    def get_frame_entry(self):
        w = self.board.frame.rect.w - \
            (self.board.grosor_borde1 + self.board.separador1)*2
        h = self.entry.frame.rect.h + \
            (self.board.grosor_borde2 + self.board.separador2)*2
        contenedor1 = VG.get_Rectangulo(self.board.colores["base"], (w, h))
        contenedor1 = VG.get_my_surface_whit_border(
            contenedor1, self.board.colores["bordes"], self.board.grosor_borde2)
        ...
        contenedor2 = VG.get_my_surface_whit_border(
            contenedor2, self.board.colores["bordes"], self.board.grosor_borde1)
        return contenedor2
    # ------------ CONSTRUCCIÓN -----------------------
