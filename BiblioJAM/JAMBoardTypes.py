...
class JAMBoardTypes(pygame.sprite.OrderedUpdates):
    def __init__(self, tipo="letras"):
        ...
        ), "cara": VG.get_blanco(), "frame": VG.get_blanco()}
        ...
        self.JAMObjects = {"frame": self.frame, "grilla": self.grilla,
                           "botones": self.botones, "text_buffer": self.text_buffer}
    ...
    def set_text(self, tipo=None, tamanio=None, color=None):
        ...
        for button in self.botones:
            ...
            if type(color) == tuple:
                cambios = True
            if cambios:
                button.set_text(tipo=tipo, tamanio=tamanio,
    ...
    def set_callback(self, callback):
        ...
        for button in self.botones:
            button.connect(callback=self.callback,
                           sonido_select=button.sonido_select)
    ...
    def set_center(self, punto=None):
        ...
        if not punto or type(punto) != tuple or type(punto[0]) != int or type(punto[1]) != int:
            posicion = (pygame.display.Info().current_w/2,
    ...
    def Reconstruye_JAMBoardTypes(self, cambios):
        ...
        if "colores" in cambios:
            for button in self.botones:
                button.set_colores(
                    colorbas=self.colores["base"], colorbor=self.colores["bordes"], colorcara=self.colores["cara"])
    ...
    def set_normaliza_tamanios_botones(self):
        ...
        if self.tipo == "especiales":
            alto = 0
            for button in self.botones:
                # Para que queden lo más pequeño posible.
                button.set_tamanios(tamanio=(0, 0))
                if button.rect.h > alto:
                    alto = button.rect.h
            for button in self.botones:
    ...
    def get_frame_and_grilla(self):
        ...
        if self.tipo == "especiales":
            ...
            for button in self.botones:  # es una sola fila
                ...
                ancho += button.rect.w
            alto = button.rect.h
            self.frame.image = VG.get_Rectangulo(
                self.colores["frame"], (ancho, alto))
            ...
            self.frame.image = VG.get_Rectangulo(
                self.colores["frame"], (ancho, alto))
            self.frame.rect = self.frame.image.get_rect()
            self.grilla = VG.get_grilla(
                self.frame.image, self.columnas, self.filas)
    # ------------ CONSTRUCCIÓN -----------------------
