...
class JAMComboBox(pygame.sprite.OrderedUpdates):
    def __init__(self):
        ...
        self.text = {"tipo": None, "tamanio": None,
                     "color": None, "font_from_file": None}
    ...
    def ordena_items(self):
        ...
        self.buttoncaption.set_tamanios(
            tamanio=(0, 0), grosorbor=1, detalle=1, espesor=1)
        self.buttonaction.set_tamanios(
            tamanio=(0, 0), grosorbor=1, detalle=1, espesor=1)
        for item in self.buttonsitems:
            item.set_tamanios(tamanio=(0, 0), grosorbor=1,
                              detalle=1, espesor=1)
        ...
        self.buttoncaption.set_tamanios(
            tamanio=(ancho, alto), grosorbor=1, detalle=1, espesor=1)
        self.buttonaction.set_tamanios(tamanio=(self.buttonaction.get_tamanio()[
                                       0], alto), grosorbor=1, detalle=1, espesor=1)
        for item in self.buttonsitems:
            item.set_tamanios(tamanio=(ancho, alto),
    ...
    def add_item(self, item, callback):
        ...
        button.set_text(
            tipo=self.text["tipo"], tamanio=self.text["tamanio"], color=self.text["color"])
        ...
        if self.text["font_from_file"]:
            button.set_font_from_file(
                self.text["font_from_file"], tamanio=self.text["tamanio"])
    ...
    def set_colores(self, colorbas=None, colorbor=None, colorcara=None):
        ...
        self.buttoncaption.set_colores(
            colorbas=colorbas, colorbor=colorbor, colorcara=colorcara)
        self.buttonaction.set_colores(
            colorbas=colorbas, colorbor=colorbor, colorcara=colorcara)
        for boton in self.buttonsitems:
    ...
    def set_text(self, tipo=None, tamanio=None, color=None):
        ...
        if not cambios:
            return
        self.buttoncaption.set_text(
            tipo=self.text["tipo"], tamanio=self.text["tamanio"], color=self.text["color"])
        self.buttonaction.set_text(
            tipo=self.text["tipo"], tamanio=self.text["tamanio"], color=self.text["color"])
        for boton in self.buttonsitems:
            boton.set_text(
                tipo=self.text["tipo"], tamanio=self.text["tamanio"], color=self.text["color"])
    ...
    def set_font_from_file(self, fuente, tamanio=None):
        ...
        if fuente:
            ...
            self.buttoncaption.set_font_from_file(
                self.text["font_from_file"], tamanio=self.text["tamanio"])
            self.buttonaction.set_font_from_file(
                self.text["font_from_file"], tamanio=self.text["tamanio"])
            for boton in self.buttonsitems:
                boton.set_font_from_file(
                    self.text["font_from_file"], tamanio=self.text["tamanio"])
    ...
    def set_tamanios(self, tamanio=(0, 0)):
        ...
        self.buttoncaption.set_tamanios(
            tamanio=tamanio)  # , grosorbor=1, detalle=1, espesor=1)
        self.buttonaction.set_tamanios(
            tamanio=(self.buttonaction.get_tamanio()[0], tamanio[1]))
        for item in self.buttonsitems:
    ...
    def set_posicion(self, punto=(0, 0)):
        if type(punto) == tuple and len(punto) == 2:
            if type(punto[0]) == int and type(punto[1]) == int:
                ...
                self.buttonaction.set_posicion(
                    punto=(self.posicion[0]+w, self.posicion[1]))
    ...
    def get_tamanio(self):
        ...
        alto = 0
        ancho = self.buttoncaption.get_tamanio(
        )[0] + self.buttonaction.get_tamanio()[0]
        if self.buttonsitems:
            for item in self.buttonsitems:
    ...
    def __init__(self, main):
        ...
        if self.main.text["font_from_file"]:
            self.set_font_from_file(
                self.main.text["font_from_file"], tamanio=self.main.text["tamanio"])
    ...
    def Run(self):
        ...
        while self.nivel == "menu_0":
            ...
            if contador == 50:
                ...
                self.widgets.set_colores(
                    colorbas=colorbase, colorbor=colorborde, colorcara=colorcara)
