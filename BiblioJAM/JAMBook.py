...
class JAMBook(pygame.sprite.OrderedUpdates):
    ...
    def Reconstruye(self, cambios):
        ...
        if "todo" in cambios:
            ...
            self.rectangulo_texto = Rectangulo_Texto(
                self, (0, 0, self.hoja.rect[2]-self.margen*1.5, self.hoja.rect[3]-self.margen*2))
    ...
    def set_posicion(self, punto=(0, 0)):
        ...
        if not type(punto) == tuple or not len(punto) == 2 or not type(punto[0]) == int or not type(punto[0]) == int:
            return
        self.posicion = punto
        self.hoja.rect.x, self.hoja.rect.y = self.posicion
        self.rectangulo_texto.set_posicion(
            punto=(self.hoja.rect.x+self.margen, self.hoja.rect.y+self.margen/2))

        x, y, w, h = self.hoja.rect  # posicionar los botones
        self.botones[0].set_posicion(
            punto=(x, y+h-self.botones[1].get_tamanio()[1]))
        self.botones[1].set_posicion(punto=(
            x+w-self.botones[1].get_tamanio()[0], y+h-self.botones[1].get_tamanio()[1]))
        self.botones[2].set_posicion(
            punto=(x+w-self.botones[2].get_tamanio()[0], y))
...
class Ejemplo(object):
    ...
    def load(self):
        ...
        # Todo se ha cargado y seteado, listo para correr el juego.
        self.estado = True
