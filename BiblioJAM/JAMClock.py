...
class JAMClock(pygame.sprite.OrderedUpdates):
    ...
    def __init__(self):
        ...
        self.datos_base = {"color": VG.get_blanco(
        ), "colorborde": COLORBAS, "tamanio": (225, 225), "grosorborde": 4}
        self.datos_agujas = {"segundero": 1, "minutero": 2,
                             "hora": 3, "color": VG.get_negro()}
    ...
    def Reconstruye(self, cambios):
        ...
        if "todo" in cambios:
            ...
            self.segundero = Aguja(
                self, self.datos_agujas["segundero"], self.datos_agujas["color"], self.retiro)
            self.minutero = Aguja(
                self, self.datos_agujas["minutero"], self.datos_agujas["color"], self.retiro*2)
            self.horario = Aguja(
                self, self.datos_agujas["hora"], self.datos_agujas["color"], self.retiro*5)
    ...
    def control_alarma(self, tiempo):
        ...
        if (h, m) == self.alarma and not self.alarma_activada and self.sonido_alarma:
            ...
            self.alarma_activada = False
            print(
                "ALARMA DESACTIVADA - VOLVERÁ A SONAR EN 24 HORAS A MENOS QUE LA DESACTIVES !!!")
    ...
    def set_alarma(self, tiempo, sound, duracion):
        ...
        # 24/11/2011 De lo contrario sólo se puede configurar la alarma 1 vez.
        self.alarma_activada = False
...
class Digital(JAMButton):
    ...
    def __init__(self, clock):
        ...
        tamanio = self.clock.datos_numeros["tamanio"] / \
            3+self.clock.datos_numeros["tamanio"]
...
class Timer(pygame.sprite.Sprite):
    ...
    def __init__(self, clock):
        ...
        self.clock = clock
        self.hora = ("00:00:00")
        superficie = VG.get_Elipse(
            self.clock.datos_agujas["color"], (self.clock.retiro*2, self.clock.retiro*2))
        self.image = VG.get_my_surface_whit_elipse_border(
            superficie, self.clock.datos_base["color"], self.clock.datos_base["grosorborde"]/2)
...
class Aguja(pygame.sprite.Sprite):
    ...
    def __init__(self, clock, grosor, color, retiro):
        ...
        superficie = VG.get_Rectangulo(
            self.clock.datos_base["color"], self.clock.datos_base["tamanio"])
...
class Base_Externa(pygame.sprite.Sprite):
    def __init__(self, clock):
        ...
        superficie = VG.get_my_surface_whit_border(
            superficie, self.clock.datos_base["colorborde"], self.clock.datos_base["grosorborde"]/2)
        ...
        self.image = VG.get_my_surface_whit_border(
            superficie2, self.clock.datos_base["colorborde"], self.clock.datos_base["grosorborde"])
        self.rect = self.image.get_rect()
...
class Numeros(pygame.sprite.OrderedUpdates):
    ...
    def Reconstruye(self, cambios):
        ...
        if "todo" in cambios:
            self.empty()
            self.cuadricula = VG.get_cuadricula(
                self.clock.base_interna.image, 11, 11)
    ...
    def set_posicion(self):
        ...
        self.numeros[0].set_posicion(
            punto=(x+self.cuadricula[8][0], y+self.cuadricula[8][1]))
        self.numeros[1].set_posicion(
            punto=(x+self.cuadricula[32][0], y+self.cuadricula[32][1]))
        self.numeros[2].set_posicion(
            punto=(x+self.cuadricula[65][0], y+self.cuadricula[65][1]))
        self.numeros[3].set_posicion(
            punto=(x+self.cuadricula[98][0], y+self.cuadricula[98][1]))
        self.numeros[4].set_posicion(
            punto=(x+self.cuadricula[118][0], y+self.cuadricula[118][1]))
        self.numeros[5].set_posicion(
            punto=(x+self.cuadricula[115][0], y+self.cuadricula[115][1]))
        self.numeros[6].set_posicion(
            punto=(x+self.cuadricula[112][0], y+self.cuadricula[112][1]))
        self.numeros[7].set_posicion(
            punto=(x+self.cuadricula[88][0], y+self.cuadricula[88][1]))
        self.numeros[8].set_posicion(
            punto=(x+self.cuadricula[55][0], y+self.cuadricula[55][1]))
        self.numeros[9].set_posicion(
            punto=(x+self.cuadricula[22][0], y+self.cuadricula[22][1]))
        self.numeros[10].set_posicion(
            punto=(x+self.cuadricula[2][0], y+self.cuadricula[2][1]))
        self.numeros[11].set_posicion(
            punto=(x+self.cuadricula[5][0], y+self.cuadricula[5][1]))
...
class Ejemplo(object):
    ...
    def setup(self):
        ...
        pygame.event.set_allowed(
            [MOUSEMOTION, MOUSEBUTTONUP, MOUSEBUTTONDOWN, KEYDOWN, KEYUP, VIDEORESIZE, VIDEOEXPOSE])
