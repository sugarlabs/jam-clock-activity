...
class PygameCanvas(gtk.EventBox):
    ...
    def mousedown(self, widget, event):
        evt = pygame.event.Event(
            pygame.MOUSEBUTTONDOWN, button=event.button, pos=(int(event.x), int(event.y)))
        try:
            pygame.event.post(evt)
        except:
            pass
    ...
    def mousemotion(self, widget, event):
        ...
        evt = pygame.event.Event(
            pygame.MOUSEMOTION, pos=self.mouse_pos, rel=rel, buttons=self.button_state)
        try:
            pygame.event.post(evt)
        except:
            pass
    ...
    def resize(self, widget, event):
        evt = pygame.event.Event(pygame.VIDEORESIZE, size=(
            event.width, event.height), width=event.width, height=event.height)
        try:
            pygame.event.post(evt)
        except:
            pass
